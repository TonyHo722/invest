...
<body>
    ...
    <p>Automated Analysis Dashboard - Generated when {time.strftime('%Y-%m-%d %H:%M:%S')}</p>
...
def fetch_and_generate(ticker_sym, company_name, current_price, mcap, metrics_list=None):
    ...
    try:
        ...
        for year in valid_columns[:4][::-1]:
            ...
            # KQJ Methodology: FCF = Operating Cash Flow - Change In Working Capital - abs(Capital Expenditure)
            if 'Operating Cash Flow' in y_cf and not pd.isna(y_cf.get('Operating Cash Flow')):
            ...
            ar = y_bs.get('Accounts Receivable', y_bs.get('Receivables', 0))
        ...
        try:
            ...
            if not q_fin.empty and len(q_fin.columns) >= 4:
                ...
                # KQJ Methodology: TTM Average Inventory & AR
                ...
                # KQJ Methodology: TTM Average Equity/Assets
